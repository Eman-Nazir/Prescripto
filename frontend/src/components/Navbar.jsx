      ...
      <div className='flex items-center gap-4'>
        ...
        {token && userData ? (
          <>
            ...
            {/* Mobile tap dropdown */}
            <div className='relative md:hidden flex items-center gap-2 cursor-pointer' onClick={() => setShowMobileDropdown(!showMobileDropdown)}>
              <img className='w-8 rounded-full' src={userData.image} alt='Profile' />
              <img className='w-2.5' src={assets.dropdown_icon} alt='Dropdown' />

              {showMobileDropdown && (
                <div className='absolute top-0 right-0 pt-14 z-20'>
                  <div className='min-w-48 bg-stone-100 rounded flex flex-col gap-4 p-4 text-base font-medium text-gray-600'>
                    <p onClick={() => { navigate("/my-profile"); setShowMobileDropdown(false); }} className='hover:text-black cursor-pointer'>My Profile</p>
                    <p onClick={() => { navigate("/my-appointments"); setShowMobileDropdown(false); }} className='hover:text-black cursor-pointer'>My Appointments</p>
                    <p onClick={() => { setToken(false); setShowMobileDropdown(false); }} className='hover:text-black cursor-pointer'>Logout</p>
                  </div>
                </div>
              )}
            </div>
          ...
          <button
            ...
            className='bg-[var(--main-blue)] text-white px-8 py-3 rounded-full font-light hidden md:block'
          >
            Create account
          </button>
        )}
